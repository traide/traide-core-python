...
from traide.observability.observability_config import ObservabilityConfig
...
def configure_observability(observability_config: ObservabilityConfig) -> None:
    configure_structlog(
        logging_config=LoggingConfig(
            ...
            loggers_to_configure=[
                ...
                LoggerConfig(name="faststream", level=LogLevel.WARNING),
                LoggerConfig(name="faststream.client", level=LogLevel.ERROR),
                LoggerConfig(name="httpx", level=LogLevel.WARNING),
            ],
        )
    )
    configure_tracing(service_name=observability_config.service_name, hostname=observability_config.hostname, tracing_type=observability_config.tracing_type)
